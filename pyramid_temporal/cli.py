...
def _import_worker_factory(worker_path: str) -> Callable:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid worker path: '{worker_path}'. "
    ...
    if not callable(worker_factory):
        raise TypeError(
            f"'{worker_path}' is not callable. "
            "Worker factory must be a function that returns a Worker instance."
        )
...
def ptemporal_worker(ini_file: str, worker_factory_path: str, log_level: str) -> None:
    ...
    try:
        ...
        if not isinstance(worker, Worker):
            raise TypeError(
                f"Worker factory must return a temporalio.worker.Worker instance, "
                f"got {type(worker)}"
            )
